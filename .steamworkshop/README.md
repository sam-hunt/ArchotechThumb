...
("Archotech") so players searching for the tech find the mod, every localized
title must contain that language's vanilla-localized term for "archotech"
(and, where natural, its body-part vocabulary) — Archotech, архотех,
アーキテック, and so on. Titles are fully localized with no English brand
...
`ArchotechThumb_SettingsCategory` Keyed value so the in-game settings header
matches the Workshop page (see the CLAUDE.md localization note).
...
English is currently the only language this mod ships (see CONTRIBUTING.md's
roster); per-language description files arrive with the initial translation
pass, tracked in TODOs.md.
