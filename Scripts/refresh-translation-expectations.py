...
# shunter.tradersstockxenogerms and its sibling repos (same L10nProbe-driven
# refresh flow); like that mod, this one is not part of a probed family — it
# is the only sidecar-bearing mod in its own boot, probed standalone.
PACKAGE_ID = "shunter.archotechthumb"
...
#     This mod has no DLC dependency and none of its defs (AT_OrbitalBeam-
#     Strike, the AT_ArchotechThumb hediff/thing/recipe family) carry a
#     MayRequire, so no DLC belongs here. Patch_RewardPools.xml's xpath
#     targets DLC-defined ThingSetMakerDefs when the relevant DLC happens to
#     be installed, but that patch adds no label/description of its own —
#     nothing this mod must translate depends on a DLC being active;
#   * hard dependencies (Harmony);
#   * this repo's own sidecar-bearing mod. This mod is not part of a probed
#     family (unlike the UniqueMeleeWeapons trio, which rides three
#     independent mods along in one boot for convenience), so the list has
#     no siblings to add;
