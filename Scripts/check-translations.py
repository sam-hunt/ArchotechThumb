...
#   <ArchotechThumb_ResetToDefaults>...</ArchotechThumb_ResetToDefaults>
...
# DLCs the sidecar must have been generated with. This mod's hard dependency
# is Harmony only (no DLC); every one of its own defs (AT_OrbitalBeamStrike,
# the AT_ArchotechThumb hediff/thing/recipe family) loads with Core alone, and
# none of them carry a MayRequire gate. The reward-pool patch (Patch_
# RewardPools.xml) xpath-matches DLC-defined ThingSetMakerDefs when the
# relevant DLC is installed, but that patch adds no label/description of its
# own — it only appends our already-translated ThingDef's defName to a
# vanilla list — so no DLC is required for the sidecar to see every
# injection point this mod is responsible for.
REQUIRED_DLCS = set()
