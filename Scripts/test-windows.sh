...
# Usage:
#   ./test-windows.sh
#   ./test-windows.sh --filter FullyQualifiedName~ArchotechThumbSettings
...
MAIN_PROJECT='Source\1.6\ArchotechThumb.csproj'
TEST_PROJECT='Tests\1.6\ArchotechThumb.Tests.csproj'

echo -e "${BLUE}=== Archotech Thumb - Windows Test Runner ===${NC}"
...
#    Stage 3 robocopies the test bin to %TEMP%\at-tests\ — a real local
...
powershell.exe -NoProfile -Command "
    ...
    \$LocalBin = Join-Path \$env:TEMP 'at-tests'
    ...
    dotnet test 'ArchotechThumb.Tests.dll' --nologo $EXTRA_ARGS
    exit \$LASTEXITCODE
" 2>&1 | tee "$TMPF"
STATUS=${PIPESTATUS[0]}
