...
Thanks for your interest in improving Archotech Thumb! Bug reports,
...
- Mod settings strings currently live in
  `1.6/Languages/English/Keyed/ArchotechThumb.xml`, keyed with the
  `ArchotechThumb_` prefix. This mod also ships its own Defs — the
  AT_OrbitalBeamStrike ability and the AT_ArchotechThumb hediff/thing/recipe
  family — so their `label`/`description`/`jobString` fields need
  `1.6/Languages/<Language>/DefInjected/<DefType>/` entries too; see the
  `translate` skill for the current list.
