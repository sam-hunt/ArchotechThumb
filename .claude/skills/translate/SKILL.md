...
- **This mod ships its own Defs**, unlike the trader-mod sibling
  ...
  - `HediffDef AT_ArchotechThumb` — `label`, `labelNoun`, `description`, plus
    ...
    that is exactly the kind of collection-nested field the sidecar exists to
    surface (see below) — do not assume it isn't translatable just because it
    sits three levels deep in `comps`.
  - `ThingDef AT_ArchotechThumb` — `label`, `description` (shares its defName
    with the HediffDef above; they resolve independently because DefInjected
    is folder-scoped by def *type*, so there is no collision).
  - `RecipeDef AT_InstallArchotechThumb` — `label`, `description`,
    `jobString`.
  So the DefInjected surface here is real and non-empty from day one — audit
  it against `Scripts/expected-injections.json`, never against this bullet
  list, since the sidecar also catches anything inherited from the `Add
  edBodyPartBase`/`BodyPartArchotechBase`/`SurgeryInstallBodyPartArtificial
...
- `<DefTypeFolder>` must be the def's resolvable type name: bare for vanilla
  ...
  Def class — so every folder today is a bare vanilla name:
  `DefInjected/AbilityDef/`, `DefInjected/HediffDef/`, `DefInjected/ThingDef/`,
  `DefInjected/RecipeDef/`. A namespace-qualified folder
  (`ArchotechThumb.<DefClass>`) would only be needed for a def whose *type*
  this mod itself defines in C# — none exist today, but the rule is
  decompile-verified and load-bearing, not organizational (see next bullet),
  so it's recorded here in advance rather than rediscovered later.
...
- DefInjected keys are `DefName.field` paths — e.g. `AT_OrbitalBeamStrike.
  label`, `AT_ArchotechThumb.description`, `AT_InstallArchotechThumb.
  jobString`. The checker's `required` subset of the
  `Scripts/expected-injections.json` sidecar is the authority for exactly
  which keys are mandatory per language, and it errors on any cross-language
  drift the moment a key appears in one language but not another.
- **A collection-nested field's key is an index path, and the index is not
  yours to guess.** The verb-giver tool's `label` on `AT_ArchotechThumb`
  lives inside `comps` (a `<li>` list) inside `tools` (another `<li>` list);
  the legal DefInjected path for it (something like
  `AT_ArchotechThumb.comps.1.tools.0.label`, exact index TBD) is only known
  by asking the sidecar — read `Scripts/expected-injections.json` for the
  real path rather than hand-deriving one from the XML's list positions.
- **Some translatable fields can exist without ever appearing in this
  repo's own XML** — inherited labels from the `AddedBodyPartBase` /
  `BodyPartArchotechBase` / `SurgeryInstallBodyPartArtificialBase` vanilla
  parents, or a C#-default string on a comp this mod's `ModInitializer`/
  settings code touches. The authority for what actually needs translating
...
- Placeholders (`{0}`, `{1}`, named args) must match English exactly per key.
  Both mod-settings keys (`ArchotechThumb_OrbitalBeamCooldown` and
...
1. Vanilla language data:
   ...
   (read entries with `tar -xOf`). Check **Core only** — this mod has no DLC
   dependency, and the vanilla defs its own defs are directly modeled on
   (`ArchotechEye`/`ArchotechArm`/`ArchotechLeg` and their install recipes,
   in `Data/Core/Defs/HediffDefs/BodyParts/Hediffs_BodyParts_Archotech.xml`)
   are Core content, not DLC-gated. Neither Biotech nor Odyssey/Royalty
   matters here; those are the trader-mod and weapon-mod siblings' domain.
...
Terms that MUST be grounded before use: archotech (the tech-level/adjective,
and the noun for the entities themselves), the "an artificial `<part>` built
by an archotech" ThingDef-description formula, "install archotech `<part>`"
...
match vanilla ability gizmos). **None of this repo's own glossary rows below
have been grounded yet.** The sibling mods' Biotech/Odyssey/Royalty-grounded
rows are irrelevant here, and no language pass in this repo has yet run a
Core-archotech-grounded generation. Treat every glossary table below as
**style/mechanics reference only** until an actual generation pass grounds
archotech-body-part vocabulary against the Core tar and records it here.
...
gun-domain content, and the trader-mod sibling (`../TradersStockXenogerms`)
generating xenogerm-domain content; this repo now joins that family.
...
independent of a mod's domain, and is reproduced below unchanged. What does
**not** carry over verbatim is the glossary *tables*: they were built for
melee-weapon and xenogerm vocabulary (weapon names, damage types, tool
labels, trait adjectives, quest vocabulary, trader-price phrasing) this mod
has limited or no use for, since its own domain is archotech body parts and
a single mod-coined ability. Each table below keeps only the rows that are
domain-independent (UI buttons, quality tiers); rows that were relevant only
to the trader-mod sibling's trader-price framing are kept for reference (the
text is still correct, native-reviewed vanilla localization) but flagged as
such rather than claimed as directly relevant here. The dropped weapon rows
are still correct too — they just live in the weapon-mod skills, which
remain the source for that vocabulary if a future feature ever needs it.
Mirror a correction the other direction too: if generating this mod's
languages surfaces a fix to a truly *shared* row (a button label, a
punctuation rule), propagate it back into the siblings.
...
repo has not yet run a Russian generation pass; add archotech-body-part rows
here once one lands.
...
| Traders will pay more/less for it. | 貿易商は高値で/低い価格でこれを買い取ります. | | Odyssey `GoldInlay`/`Ugly` descs — reuse verbatim; kept for family reference; not used by this mod's own text |
...
add archotech-body-part rows here once one lands.
...
has not yet run a Simplified Chinese generation pass; add archotech-body-part
...
| Traders will pay more/less for it. | 상인들이 더 높은 값을 쳐줍니다. / 상인들은 더 적은 돈을 쳐줍니다. | | Odyssey `GoldInlay`/`Ugly` — reuse verbatim; kept for family reference; not used by this mod's own text |
...
different tar subset. **Ground Archotech Thumb's own body-part terms
independently against the Core tar rather than assuming a weapon- or
trader-mod sibling's word for an adjacent concept transfers.**
...
archotech-body-part rows here once one lands.
...
a German generation pass; add archotech-body-part rows here once one lands.
...
| Traders will pay more/less for it. | `Los comerciantes pagarán más por ella.` / `… menos por ella.` | | Odyssey `GoldInlay`/`Ugly` — reuse verbatim; kept for family reference; not used by this mod's own text |
...
| Traders will pay more/less for it. | `Les commerçants en paieront un prix plus élevé.` / `Les commerçants en paieront moins cher.` | | Odyssey `GoldInlay`/`Ugly` — reuse verbatim; kept for family reference; not used by this mod's own text |
...
French generation pass; add archotech-body-part rows here once one lands.
...
| Traders will pay more/less for it. | `Comerciantes pagarão mais por ela.` / `Comerciantes pagarão menos por ela.` | | Odyssey `GoldInlay`/`Ugly` — reuse verbatim; kept for family reference; not used by this mod's own text |
...
- **A def field's official label can differ across the def *types* that
  ...
  the full pattern). This mod defines both a `HediffDef` and a `ThingDef`
  under the same defName (`AT_ArchotechThumb`) — the installed-part hediff
  and the loose item are separate translation targets even though they share
  a name, so confirm which def *type*'s label you're translating, not just
  the defName.
- **When two vanilla files disagree, prefer the nearer analog, not the
  ...
  `púrpura`. For this mod that means: if Core's own archotech-body-part
  Keyed/DefInjected data (`ArchotechEye`/`ArchotechArm`/`ArchotechLeg` and
  their install recipes) ever disagrees with more generic Core item
  vocabulary on a term, the archotech-body-part data wins — it's the nearer
  analog.
- **Don't spend a vanilla word on the wrong slot.** Map any concept this
  ...
  don't reuse a word Core already spends on a specific archotech body part
  or ability-gizmo concept for something else), and coin only for what's
  genuinely left over.
...
2. Enumerate the English Keyed keys in `ArchotechThumb.xml` and the
   DefInjected surface from `Scripts/expected-injections.json` (four defs:
   `AT_OrbitalBeamStrike`, `AT_ArchotechThumb` as both HediffDef and ThingDef,
   `AT_InstallArchotechThumb`) — confirm the current set against the sidecar
   rather than assuming this list stays accurate.
3. Extract the vanilla tar for the target language into the scratchpad;
   build a term list for the grounded terms above (Core only).
