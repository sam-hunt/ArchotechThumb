...
Prepare and publish a new release for Archotech Thumb.
...
The post-build `StageMod` target atomically wipes and re-stages the deployed
mod folder, so no separate clean step is needed.
...
- Boots the freshly deployed build once on its pinned minimal list (no
  ...
  attributed to Archotech Thumb. With no optional integrations, this is a
...
- Rebuild (`dotnet build ArchotechThumb.sln -c Release`) so the deployed
